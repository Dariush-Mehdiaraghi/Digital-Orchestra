...
})({"3TKuQ":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "387d1611793385f0";
"use strict";
...
},{}],"iLfKW":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "receiverSketch", ()=>receiverSketch);
parcelHelpers.export(exports, "startMicrophoneInput", ()=>startMicrophoneInput);
...
let frequencyFound;
let mic;
const micFFT = new _tone.FFT(8192);
const receiverSketch = function(p) {
    p.role = "receiver";
    p.mic;
    p.fft;
    p.peakBuffer = [];
    ...
    p.draw = function() {
        ...
        p.stroke((0, _constantsJs.mySecondaryColor));
        p.background((0, _constantsJs.myBackgroundColor));
        let indexOfMaxValue = indexOfMax(spectrum);
        let peakFreq = round_to_precision(indexOfMaxValue * 22050 / spectrum.length, 100);
        if (peakFreq != undefined && peakFreq > 0) {
            p.peakBuffer[p.peakCount % p.peakDuration] = peakFreq;
            p.peakCount++;
        }
        let bufferSum = 0;
        for(let i1 = 0; i1 < p.peakDuration; i1++){
            bufferSum += p.peakBuffer[i1];
            if (bufferSum / (p.peakBuffer.length - 1) == peakFreq && peakFreq > 0 && -1 * spectrum[indexOfMaxValue] > p.peakMinAmp && frequencyFound != peakFreq && peakFreq > 1900) {
                frequencyFound = peakFreq;
                (0, _socketJs.socket).emit("foundFreq", frequencyFound);
                console.log("〰️ found frequency: " + frequencyFound + "Hz");
            }
        }
        p.beginShape();
        let x = 0;
        for(i = 0; i < spectrum.length; i++)p.vertex(p.map(p.abs(spectrum[2 ^ i]), 0, 255, p.width, 0), i);
        p.endShape();
        // p.noLoop();
        if (spectrum[indexOfMaxValue] > p.peakMinAmp) {
            p.fill((0, _constantsJs.mySecondaryColor));
    ...
    function indexOfMax(arr) {
        ...
        var max = arr[0];
        var maxIndex = 0;
        for(var i1 = 1; i1 < arr.length; i1++)if (arr[i1] > max) {
            maxIndex = i1;
            max = arr[i1];
        }
        return maxIndex;
...
const oldOne = ()=>{
    var webaudio_tooling_obj = function() {
        ...
        function process_microphone_buffer(event) {
            var i1, N, inp, microphone_output_buffer;
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./constants.js":"ib2Pl","tone":"2tCfN","./socket.js":"8guOI"}]},["3TKuQ","iLfKW"], "iLfKW", "parcelRequirecba9")

//# sourceMappingURL=index.793385f0.js.map
