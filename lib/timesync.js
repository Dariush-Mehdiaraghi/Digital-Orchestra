...
export function create(options) {
  var timesync = {
    ...
    sync: function () {
      ...
      return Promise.all(peers.map(peer => timesync._syncWithPeer(peer))).then(function (all) {
    ...
    for (var prop in options) {
      if (options.hasOwnProperty(prop)) {
        if (prop === 'peers' && typeof options.peers === 'string') {
          ...
          timesync.options.peers = options.peers.split(',').map(peer => peer.trim()).filter(peer => peer !== '');
        } else {
